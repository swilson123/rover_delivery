	...
	if (message.name == 'HEARTBEAT') {
		...
		if (message.autopilot == 3) {
			...
			if (rover.flight_data.robot_flight_mode != rover.FlightModes[rover.robot_data.HEARTBEAT.custom_mode]) {
				...
				if (rover.flight_data.robot_flight_mode == 'Auto') {
		...
		rover.altitude.relative_alt_meters = rover.robot_data.GLOBAL_POSITION_INT.relative_alt / 1000;
		console.log('pixhawk_message_handler: MISSION_COUNT - ', rover.mission.mission_count);
		...
		console.log('pixhawk_message_handler: MISSION_ITEM_REACHED - ', message);
		...
		if (rover.mission.mission_count == rover.mission.current_mission_seq + 1) {
			console.log('pixhawk_message_handler: Mission Finished');
