var avoid_object = function (rover) {
	...
	if (rover.flight_data.mav_state == 'MAV_STATE_ACTIVE' && rover.mission.mission_count > 0) {
		...
		if (!rover.mission.path_clear) {
			...
			//is there a green zone facing the waypoint?
			for (var i = 0; i < rover.zones.length; i++) {
				if (rover.zones[i].light == "green") {
					...
					let angle_diff = Math.abs(((waypoint_bearing - zone_center + 540) % 360) - 180);
					if (angle_diff < 30) { // within 30 degrees of waypoint bearing
						console.log(`Green zone ${rover.zones[i].zone} is facing waypoint bearing ${waypoint_bearing}`);
			...
			if (!rover.mission.path_clear) {
				if (rover.zones[10].light !== "green" || rover.zones[11].light !== "green") {
					...
					if (nearest_green) {
						...
						rover.guided_mode_command(rover, 0, 0, 0, yaw_to_green);
					} else {
					...
					console.log('Path ahead is clear, moving forward. 1 meter');
					rover.guided_mode_command(rover, 1, 0, 0, 0);
				}
			}
